%% data aquisition 
% code modified from "ephysJC4" to include a second DAQ board for more output channels
...
for a=1:params.numRepeats ;
    ...
    % set daq output
    outputData{a} = zeros(fromEphys.lt,3) ; % ao0
    ...
    if strcmp(params.OdorValveStimType,'MichelsonContrast') ; % use 2 valves to control constrast steps and 1 for background
        outputData{a}(params.OdorPulseInitTime*params.sampleRate:(params.OdorPulseInitTime+params.OdorPulseDur)*params.sampleRate,1)= params.OdorPulseAmp ;
    ...
    if strcmp(params.VextStimType,'BackgroundOdorAlternate') ;
         outputData{a}(params.InjPulseInitTime*params.sampleRate:(params.InjPulseInitTime+params.InjPulseDur)*params.sampleRate,3)= params.InjPulseAmp*rem(a+1,2) ; %step off for odd, on for even
    end
