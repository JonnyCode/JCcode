function ForIgor = PNadaptationAnalysisE(Input,A)

% this function is adapted from "PNadaptationAnalysisD", but uses a fixed odor response time across all responses.
...
OdorRspTime = 1.5 ; % (sec) time post odor pulse start which odor response is assessed
...
iorb = iopb ;
iore = iorb + OdorRspTime*sampleRate ;
...
for a = 1:NumBackgrounds ; % for each background
    for b = 1:NumConcentrations ; % for concentration
        if NumTrials(a,b)>0 ;
            for c = 1:NumTrials(a,b) ;
                Pot{a}{b}(c) = mean(vData{a}{b}(c,iorb:iore)) ; %mV (odor response depol start: odor response depol end)
...
for a = 1:NumBackgrounds ; % for each background
    for b = 1:NumConcentrations ; % for concentration
        if NumTrials(a,b)>0 ;
            for c = 1:NumTrials(a,b) ;
                RespSpikeRate{a}{b}(c) = mean(Psth{a}{b}(c,iorb:iore)); %mV (odor response depol start: odor response depol end)
            end
...
for a = 1:NumBackgrounds ; % for each background
    for b = 1:NumConcentrations ; % for concentration
        if NumTrials(a,b)>0 ;
            ...
            hold on
            plot(time(1,[iorb,iore]),vData_mean{a}{b}(1,[iorb,iore]),'o','Color',colorMat{NumConcentrations}(a,:))
...
for a = 1:NumBackgrounds ; % for each background
    for b = 1:NumConcentrations ; % for concentration
        if NumTrials(a,b)>0 ;
            ...
            title(num2str(Concentrations(b)))
            hold on
            plot(time(1,[iorb,iore]),Psth_mean{a}{b}(1,[iorb,iore]),'o','Color',colorMat{NumConcentrations}(a,:))
