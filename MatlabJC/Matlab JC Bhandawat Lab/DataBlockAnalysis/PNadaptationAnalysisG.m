function ForIgor = PNadaptationAnalysisG(Input,A)
...
BgConcentrations = str2num(Input(A).(id3)) ;
...
for a = 1:NumBackgrounds ;
    for b = 1:NumConcentrations ; % for concentration
        if NumTrials(a,b)>0 ;
            ...
            [m,mi] = min(Psth_mean{a}{b}(iope:iope+sampleRate)) ; % min point of mean vdata within a second of odor pulse offset
            iohb(a,b) = mi-1+iope - OdorRspTime*sampleRate ; % point of odor pulse begining
            iohe(a,b) = mi-1+iope + OdorRspTime*sampleRate ; % point of odor pulse end
        end
    end
end
...
for a = 1:NumBackgrounds ; % for each background
    for b = 1:NumConcentrations ; % for concentration
        if NumTrials(a,b)>0 ;
            for c = 1:NumTrials(a,b) ;
                Vpulse{a}{b}(c) = mean(vData{a}{b}(c,iorb(a,b):iore(a,b))) ; %mV (odor response depol start: odor response depol end)
                VpulseHyp{a}{b}(c) = mean(vData{a}{b}(c,iohb(a,b):iohe(a,b))) ; %mV (odor response hyp start: odor response hyp end)
            end
...
for a = 1:NumBackgrounds ; % for each background
    for b = 1:NumConcentrations ; % for concentration
        if NumTrials(a,b)>0 ;
            for c = 1:NumTrials(a,b) ;
                SRpulse{a}{b}(c) = mean(Psth{a}{b}(c,iorb(a,b):iore(a,b))); %mV (odor response depol start: odor response depol end)
                SRpulseHyp{a}{b}(c) = mean(Psth{a}{b}(c,iohb(a,b):iohe(a,b))); %mV (odor response hyperpol start: odor response hyperpol end)
            end
...
for a = 1:NumBackgrounds ; % for each background
    identifier = ['SRminBgMean','LogBg',num2str(abs(log10(BgConcentrations(a)))),'cell',num2str(A)] ;
    ForIgor.(identifier) = SRpulseMinBg_norm_mean(a,:) ;
    
    identifier = ['SRminBgStd','LogBg',num2str(abs(log10(BgConcentrations(a)))),'cell',num2str(A)] ;
    ForIgor.(identifier) = SRpulseMinBg_norm_std(a,:) ;   
end

for a = 1:NumBackgrounds ; % for each background
    for b = 1:NumConcentrations ; % for concentration 
        if NumTrials(a,b)>0 ;
            identifier = ['Psth','LogPulse',num2str(abs(log10(Concentrations(b)))),'LogBg',num2str(abs(log10(BgConcentrations(a)))),'cell',num2str(A)] ;
            ForIgor.(identifier) = Psth_mean{a}{b} ;

            identifier = ['PsthMinBg','LogPulse',num2str(abs(log10(Concentrations(b)))),'LogBg',num2str(abs(log10(BgConcentrations(a)))),'cell',num2str(A)] ;
            ForIgor.(identifier) = PsthMinBg_mean{a}{b} ;        

            identifier = ['vData','LogPulse',num2str(abs(log10(Concentrations(b)))),'LogBg',num2str(abs(log10(BgConcentrations(a)))),'cell',num2str(A)] ;
            ForIgor.(identifier) = vData_mean{a}{b} ;

            identifier = ['vDataMinBg','LogPulse',num2str(abs(log10(Concentrations(b)))),'LogBg',num2str(abs(log10(BgConcentrations(a)))),'cell',num2str(A)] ;
