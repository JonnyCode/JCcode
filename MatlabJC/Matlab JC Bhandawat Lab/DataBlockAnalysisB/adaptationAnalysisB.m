function ForIgor = adaptationAnalysisB(Input,A,AnalysisFlag)

% adaptation of "adaptationAnalysisA" 
% focuses on pair corrected data
...
for DB = 1:length(A) ; % for each cell
    ...
    if strcmp(AnalysisFlag,'ORNdata')
        spikeDetectionParameters.numClusters = 1 ; % (1)
        spikeDetectionParameters.numPCs = 2 ; % (2)
        spikeDetectionParameters.NegDiffThreshStd = 1.75 ; % (1.25) 
    elseif strcmp(AnalysisFlag,'PNdata')
        spikeDetectionParameters.absRefTime = 0.002 ; % sec (.002)
        spikeDetectionParameters.minRise = 0.3 ; % mV (.3)
        spikeDetectionParameters.minFall = 0.15 ; % mV (.15)
...
% graphs for single page focus
    ...
    PopData.BgConcentrationRangeNum = [10^-7,10.^[-8:-3]] ; % numerical version, control changed to 10^-x and wash not included
...
%     end
    ...
    clearvars -except Input A ForIgor PopData AnalysisFlag  
 end % Data block loop
...
for a = 1:length(PopData.BgConcentrationRange) ; % for each background
    if ~isempty(PopData.SRbgMeanMinRest_ApMean{a}) ; % if there are DB at this background
        for b = 1:length(PopData.ConcentrationRange) ; % for each pulse strength
            ...
            PopData.Psth_mean_AdWMean{a}(b,:) = nansum(PopData.Psth_mean{a}{b}./PopData.Psth_sem{a}{b})./nansum(1./PopData.Psth_sem{a}{b}) ;
        end
    end
end
...
% finding threshold on dose-response with linear interpolation
Threshold = 2*max(PopData.SRbgStd_ApMean_AdbWMean) ;
for a = 1:length(PopData.BgConcentrationRange)-1 ; % for each background (but 'wash')
    if sum(~isnan(PopData.SRpulseMeanMinBg_mean_DivCon_AdbWMean_TimCon(a,:)))>1 ;
        RealNumi = find(~isnan(PopData.SRpulseMeanMinBg_mean_DivCon_AdbWMean_TimCon(a,:))==1) ;
        PopData.ThresholdOdor(a) = interp1(PopData.SRpulseMeanMinBg_mean_DivCon_AdbWMean_TimCon(a,RealNumi),log10(PopData.ConcentrationRange(RealNumi)),Threshold) ; % linear interpolation
    else
        PopData.ThresholdOdor(a) = nan ;
    end
end

% fit dose response with sigmoid (this fit is hacky, not sure the proper way to do this)
for a = 1:length(PopData.BgConcentrationRange)-1 ;
    InitCoefs = [-4,2] ; % initial estimated mean and variance
    PopData.ConcentrationRangeFit = [min(log10(PopData.ConcentrationRange)):.1:max(log10(PopData.ConcentrationRange))] ; % interpolated data range
    if sum(~isnan(PopData.SRpulseMeanMinBg_mean_DivCon_AdbWMean_TimCon(a,:)))>1 ; % if there is data
        ...
        FitParams.amp = (max(PopData.SRpulseMeanMinBg_mean_DivCon_AdbWMean_TimCon(a,:))+PopData.SRpulseMeanMinBg_mean_DivCon_AdbWMean_TimCon(a,Lasti))/2 ; % amp of sig fit (max+last point)/2         
        FitParams.x = log10(PopData.ConcentrationRange) ; % data x values
        FitParams.Xinterp = PopData.ConcentrationRangeFit ; % interpolated data range
        PopData.FitCoefs{a} = nlinfit(FitParams,PopData.SRpulseMeanMinBg_mean_DivCon_AdbWMean_TimCon(a,:),@SigFun,InitCoefs) ; % find fit params
        FitParams.x = FitParams.Xinterp ; % temporarally change x to get interpolated data
        PopData.SigFit(a,:) = SigFun(PopData.FitCoefs{a},FitParams) ; % generate curve
    else
...
% check fit of weber-fechner function agaist adaptation factor vs. background
WfX = 10.^[-9:.1:-2] ;
for a=1:length(PopData.ConcentrationRange) ;
    [m,mi] = min(abs(.5-PopData.SRpulseMeanMinBg_mean_DivCon_AdbWMean(1:end-1,a))) ; % closest to half adapting background
    InitCoefs = [PopData.BgConcentrationRangeNum(mi),1] ;
    PopData.WfFitCoefs{a} = nlinfit(PopData.BgConcentrationRangeNum, PopData.SRpulseMeanMinBg_mean_DivCon_AdbWMean(1:end-1,a)', @WeberFechner, InitCoefs) ;
    PopData.WfFit(a,:) = WeberFechner(PopData.WfFitCoefs{a},WfX) ;
end
...
for a = 1:length(PopData.ConcentrationRange) ; % for each pulse(but 'wash')
    figure
    for b = 1:length(PopData.BgConcentrationRange)-1 ; % for each bg concentration but wash
        for c = 1:length(PopData.NumTrials{b}(:,a)) ; 
            if nansum(PopData.NumTrials{b}(:,a))>0 ; % if there is data in at this pulse and background
                subplot(length(PopData.BgConcentrationRange)-1,1,b) ;
...
for a = 2:length(PopData.BgConcentrationRange)-1 ; % for each background (but 'wash')
    for b = 1:length(PopData.ConcentrationRange) ; % for each pulse concentration
        for c = 1:length(PopData.NumTrials{a}) ;
            if PopData.NumTrials{a}(c,b)>0 ; % if there is data in this cell at this background
                subplot(length(PopData.BgConcentrationRange)-2,length(PopData.ConcentrationRange),sub2ind([length(PopData.ConcentrationRange),length(PopData.BgConcentrationRange)-2],b,a-1))
...
for a = 1:length(PopData.BgConcentrationRange)-1 ; % for each background (but 'wash')
    if ~isempty(PopData.SRbgMeanMinRest_ApMean{a}) ; % if there are DB at this background
        for b = 1:length(PopData.SRbgMeanMinRest_ApMean{a}) ; % for each DB at that background
            ...
            hold on
            %plot(PopData.ConcentrationRangeFit,PopData.SigFit(a,:),'Color',colorMat{5}(a,:))
            ylabel('SRpulseMeanMinBg')
            xlabel('log(pulse)')
        end
    end
...
figure % sigmoid mean and variance fits as function of background with persect contrast adaptation fits
FitCoefsMat = cell2mat(PopData.FitCoefs') ;
subplot(1,2,1)
plot(PopData.BgConcentrationRangeNum, FitCoefsMat(:,1),'*')
hold on
plot(PopData.BgConcentrationRangeNum,log10(PopData.BgConcentrationRangeNum)*.5,'r')

subplot(1,2,2)
plot(PopData.BgConcentrationRangeNum, FitCoefsMat(:,2),'*')


figure % weber-fechner fits
for a=1:length(PopData.ConcentrationRange) ; % for each pulse strength
    subplot(1,length(PopData.ConcentrationRange),a)
    errorbar(PopData.BgConcentrationRangeNum, PopData.SRpulseMeanMinBg_mean_DivCon_AdbWMean(1:end-1,a),PopData.SRpulseMeanMinBg_mean_DivCon_AdbSem(1:end-1,a),PopData.SRpulseMeanMinBg_mean_DivCon_AdbSem(1:end-1,a))
    hold on
    plot(WfX,PopData.WfFit(a,:),'r')
    set(gca,'Xscale','log')
end
