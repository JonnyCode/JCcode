function ForIgor = adaptationAnalysisD(Input,A,AnalysisFlag)

% adaptation of "adaptationAnalysisB" 
% got rid of backgrounds not used (-8,-3, and wash)
% remove analysis not used 
% fixed psth to avoid small bin artifacts 
% add flag to use different time peaks 
% add sigmoid fit analyses 
...
for DB = 1:length(A) ; % for each cell
    ...
    SinglePeakFlag = false ; % if true than peak pnt is the same for all cells and types (peak of mean PN response across cells at a given bg and pulse combo)
    ...
    if SinglePeakFlag == true ; % if using same point for all cells
        PreviousDetect = load([PopData.PNpsthPeakPntPath,'PNpeaksPnts']) ; 
        ...
        for a = 1:NumBackgrounds ;
            for b = 1:NumConcentrations ; % for concentration
                if NumTrials(a,b)>0 ; 
                    SRiorb(a,b) = PreviousDetect.AllCellPeakPnt(Bgi(a),PopPulsei(b)) - OdorRspPnts ; % point of odor pulse begining
                    SRiore(a,b) = PreviousDetect.AllCellPeakPnt(Bgi(a),PopPulsei(b)) +  OdorRspPnts ; % point of odor pulse end
                end
            end
        end
        
    else % if using different points for cells
        for a = 1:NumBackgrounds ;
            for b = 1:NumConcentrations ; % for concentration
                if NumTrials(a,b)>0 ;         
...
% peak time of psth average 
% odor response time
iopb = 150000 ; % odor pulse begining (when odor pulse begins at 15 seconds)
for a = 1:length(PopData.BgConcentrationRange) ; % for each background
    for b = 1:length(PopData.ConcentrationRange) ; % for each pulse strength
        if ~isnan(sum(PopData.PsthMinRest_mean_AdWMean{a}(b,:))) ; % if there is data
            [m,mi] = max(PopData.PsthMinRest_mean_AdWMean{a}(b,iopb:iopb+10000)) ; % max point of mean psth within a second of odor pulse onset
            AllCellPeakPnt(a,b) = mi+iopb ;
        else
            AllCellPeakPnt(a,b) = nan ;
        end
    end
end
if strcmp(AnalysisFlag,'PNdata') ; % save the peaks only for PN data - these will be used.  not the ORN peaks
    save([PopData.PNpsthPeakPntPath,'PNpeaksPnts'],'AllCellPeakPnt') ; % save peak pnts
end
...
for a = 1:length(PopData.BgConcentrationRange) ;
    ...
    if sum(~isnan(PopData.SRpulseMeanMinBg_mean_DivCon_AdbWMean_TimCon(a,:)))>1 ; % if there is data
        ...
        FitParams.MaxSat = (max(PopData.SRpulseMeanMinBg_mean_DivCon_AdbWMean_TimCon(a,:))+PopData.SRpulseMeanMinBg_mean_DivCon_AdbWMean_TimCon(a,Lasti))/2 ; % amp of sig fit (max+last point)/2         
        FitParams.MinSat = (min(PopData.SRpulseMeanMinBg_mean_DivCon_AdbWMean_TimCon(a,:))+PopData.SRpulseMeanMinBg_mean_DivCon_AdbWMean_TimCon(a,Firsti))/2 ; % amp of sig fit (min+first point)/2
        ...
        PopData.FitCoefs{a} = nlinfit(FitParams,PopData.SRpulseMeanMinBg_mean_DivCon_AdbWMean_TimCon(a,:),@SigFunInterp2free2fixed,InitCoefs) ; % find fit params
        FitParams.x = FitParams.Xinterp ; % temporarally change x to get interpolated data
        PopData.SigFit(a,:) = SigFunInterp2free2fixed(PopData.FitCoefs{a},FitParams) ; % generate curve
    else
...
InitCoefs = [-6,FitCoefsMat(1,1),1] ;
PopData.HockeyStick_FitCoefs = nlinfit(log10(PopData.BgConcentrationRangeNum), FitCoefsMat(:,1)',@HockeyStick,InitCoefs) ;
PopData.HockeyStickfit = HockeyStick(PopData.HockeyStick_FitCoefs,PopData.BgConcentrationRangeNum) ;
...
subplot(1,2,1)
plot(log10(PopData.BgConcentrationRangeNum), FitCoefsMat(:,1),'*')
hold on
plot(log10(PopData.BgConcentrationRangeNum),PopData.HockeyStickfit,'r')

subplot(1,2,2)
plot(log10(PopData.BgConcentrationRangeNum), FitCoefsMat(:,2),'*')
