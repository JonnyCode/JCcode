function ForIgor = PNadaptationAnalysisI(Input,A)

% this function will analyze PN pulse responses +/- a background odor that
% is triggered during the trial
% similar to H but uses contains data block loop and performs population
% analses
...
for DB = 1:length(A) ; % for each cell
    ...
    % odor pulse response initiation time
     for a = 1:NumBackgrounds ; % for each background
        for b = 1:NumConcentrations ; % for concentration
            if NumTrials(a,b)>0 ;
                Psth_mean_max(a,b) = max(Psth_mean{a}{b}(ibpb+bgTransPnts:iopb)) ;
                Psth_mean_std(a,b) = std(Psth_mean{a}{b}(ibpb+bgTransPnts:iopb)) ;
                ti = find(Psth_mean{a}{b}(iopb:end)>Psth_mean_max(a,b)+Psth_mean_std(a,b),1,'first') ;
                if ~isempty(ti)
                    STpulse(a,b) = ti/sampleRate ;
                else
                    STpulse(a,b) = (SRiorb(a,b)-iopb)/sampleRate ;
                end        
            else
                STpulse(a,b) = nan ;
            end
...
%     end
      ...
      % pulse response times as function of pulse concentration
    figure
    for a = 1:NumBackgrounds ;
        plot(log10(Concentrations),STpulse(a,:),'Color',colorMat{NumConcentrations}(a,:))
        hold on
    end
    ylabel('pulse response time (sec)')
...
%     
%     for a = 1:NumBackgrounds ; % for each background, 
%         identifier = ['SRminBgMean','Bg',BgConcentrations{a},'cell',num2str(A(DB))] ;
%         ForIgor.(identifier) = SRpulseMinBg_norm_mean(a,:) ;
%         
%         identifier = ['SRminBgSem','Bg',BgConcentrations{a},'cell',num2str(A(DB))] ;
%         ForIgor.(identifier) = SRpulseMinBg_norm_sem(a,:) ;  
%         
%         identifier = ['VminBgMean','Bg',BgConcentrations{a},'cell',num2str(A(DB))] ;
%         ForIgor.(identifier) = VpulseMinBg_norm_mean(a,:) ;
%         
%         identifier = ['VminBgSem','Bg',BgConcentrations{a},'cell',num2str(A(DB))] ;
...
%         for b = 1:NumConcentrations ; % for concentration 
%             if NumTrials(a,b)>0 ;
%                 identifier = ['Psth','LogPulse',num2str(abs(log10(Concentrations(b)))),'Bg',BgConcentrations{a},'cell',num2str(A(DB))] ;
%                 ForIgor.(identifier) = Psth_mean{a}{b} ;
% 
%                 identifier = ['PsthMinBg','LogPulse',num2str(abs(log10(Concentrations(b)))),'Bg',BgConcentrations{a},'cell',num2str(A(DB))] ;
%                 ForIgor.(identifier) = PsthMinBg_mean{a}{b} ;        
% 
%                 identifier = ['vData','LogPulse',num2str(abs(log10(Concentrations(b)))),'Bg',BgConcentrations{a},'cell',num2str(A(DB))] ;
%                 ForIgor.(identifier) = vData_mean{a}{b} ;
% 
%                 identifier = ['vDataMinBg','LogPulse',num2str(abs(log10(Concentrations(b)))),'Bg',BgConcentrations{a},'cell',num2str(A(DB))] ;
...
%     PopData.ConcentrationRange = 10.^[-8:-3] ;    
...
%                 PopData.vData_mean{a}{b} = nan(length(A),length(time)) ;
%                 PopData.vDataMinBg_mean{a}{b} = nan(length(A),length(time)) ;
%             end
%         end
%     end
%     for a = 1:NumBackgrounds ; % for each background
...
%                 PopData.vData_mean{PopBgi(a)}{PopPulsei(b)}(DB,:) = vData_mean{a}{b} ;
%                 PopData.vDataMinBg_mean{PopBgi(a)}{PopPulsei(b)}(DB,:) = vDataMinBg_mean{a}{b} ;
%             end
%         end
%     end 
%     
...
%             PopData.VpulseMinBg_norm_mean{a} = nan(length(A),length(PopData.ConcentrationRange)) ;
%             PopData.VpulseMinBg_norm_sem{a} = nan(length(A),length(PopData.ConcentrationRange)) ;
%         end
%     end
%     for a = 1:NumBackgrounds ; % for each background
...
%         PopData.VpulseMinBg_norm_mean{PopBgi(a)}(DB,PopPulsei) = VpulseMinBg_norm_mean(a,:) ;
%         PopData.VpulseMinBg_norm_sem{PopBgi(a)}(DB,PopPulsei) = VpulseMinBg_norm_sem(a,:) ;
%     end
%     
...
%                 PopData.VpulseMinBg_mean_paired{a}{b} = nan(2,length(A))  ; 
%                 PopData.VpulseMinBg_sem_paired{a}{b} = nan(2,length(A))  ;  
%             end
%         end
%     end
...
%                 PopData.VpulseMinBg_mean_paired{PopBgi(a)}{PopPulsei(b)}(:,DB) = [VpulseMinBg_mean(1,b);VpulseMinBg_mean(a,b)] ; 
%                 PopData.VpulseMinBg_sem_paired{PopBgi(a)}{PopPulsei(b)}(:,DB) = [VpulseMinBg_sem(1,b);VpulseMinBg_sem(a,b)] ;  
%             end    
%         end
%     end
%     
...
%     identifier = ['SRminBgMean','Bg',PopData.BgConcentrationRange{a},'AllMean'] ;
%     ForIgor.(identifier) = nanmean(PopData.SRpulseMinBg_norm_mean{a},1) ;
% 
%     identifier = ['SRminBgSem','Bg',PopData.BgConcentrationRange{a},'AllMean'] ;
%     ForIgor.(identifier) = nanmean(PopData.SRpulseMinBg_norm_sem{a},1) ;  
% 
%     identifier = ['VminBgMean','Bg',PopData.BgConcentrationRange{a},'AllMean'] ;
%     ForIgor.(identifier) = nanmean(PopData.VpulseMinBg_norm_mean{a},1) ;
% 
%     identifier = ['VminBgSem','Bg',PopData.BgConcentrationRange{a},'AllMean'] ;
%     ForIgor.(identifier) = nanmean(PopData.VpulseMinBg_norm_sem{a},1) ; 
% end
% 
% for a = 1:length(PopData.SRpulseMinBg_norm_mean) ; % for each background assessed in the population 
%     identifier = ['SRminBgMean','Bg',PopData.BgConcentrationRange{a},'AllSem'] ;
%     ForIgor.(identifier) = nanstd(PopData.SRpulseMinBg_norm_mean{a},[],1)./sqrt(sum(~isnan(PopData.SRpulseMinBg_norm_mean{a}),1)) ;
% 
%     identifier = ['SRminBgSem','Bg',PopData.BgConcentrationRange{a},'AllSem'] ;
%     ForIgor.(identifier) = nanstd(PopData.SRpulseMinBg_norm_sem{a},[],1)./sqrt(sum(~isnan(PopData.SRpulseMinBg_norm_sem{a}),1)) ; 
% 
%     identifier = ['VminBgMean','Bg',PopData.BgConcentrationRange{a},'AllSem'] ;
%     ForIgor.(identifier) = nanstd(PopData.VpulseMinBg_norm_mean{a},[],1)./sqrt(sum(~isnan(PopData.VpulseMinBg_norm_mean{a}),1)) ;
% 
%     identifier = ['VminBgSem','Bg',PopData.BgConcentrationRange{a},'AllSem'] ;
%     ForIgor.(identifier) = nanstd(PopData.VpulseMinBg_norm_sem{a},[],1)./sqrt(sum(~isnan(PopData.VpulseMinBg_norm_sem{a}),1)) ; 
% end
% 
% for a = 1:length(PopData.Psth_mean) ; % for each background
%     for b = 1:length(PopData.Psth_mean{a}) ; % for concentration 
% 
%             identifier = ['Psth','LogPulse',num2str(abs(log10(PopData.ConcentrationRange(b)))),'Bg',PopData.BgConcentrationRange{a},'AllMean'] ;
%             ForIgor.(identifier) = nanmean(PopData.Psth_mean{a}{b},1) ;
% 
%             identifier = ['PsthMinBg','LogPulse',num2str(abs(log10(PopData.ConcentrationRange(b)))),'Bg',PopData.BgConcentrationRange{a},'AllMean'] ;
%             ForIgor.(identifier) = nanmean(PopData.PsthMinBg_mean{a}{b},1) ;        
% 
%             identifier = ['vData','LogPulse',num2str(abs(log10(PopData.ConcentrationRange(b)))),'Bg',PopData.BgConcentrationRange{a},'AllMean'] ;
%             ForIgor.(identifier) = nanmean(PopData.vData_mean{a}{b},1) ;
% 
%             identifier = ['vDataMinBg','LogPulse',num2str(abs(log10(PopData.ConcentrationRange(b)))),'Bg',PopData.BgConcentrationRange{a},'AllMean'] ;
%             ForIgor.(identifier) = nanmean(PopData.vDataMinBg_mean{a}{b},1) ;   
%     end
% end
% 
% for a = 1:length(PopData.Psth_mean) ; % for each background
%     for b = 1:length(PopData.Psth_mean{a}) ; % for concentration 
% 
%             identifier = ['Psth','LogPulse',num2str(abs(log10(PopData.ConcentrationRange(b)))),'Bg',PopData.BgConcentrationRange{a},'AllSem'] ;
%             ForIgor.(identifier) = nanstd(PopData.Psth_mean{a}{b},[],1)./sqrt(sum(~isnan(PopData.Psth_mean{a}{b}),1)) ;
% 
%             identifier = ['PsthMinBg','LogPulse',num2str(abs(log10(PopData.ConcentrationRange(b)))),'Bg',PopData.BgConcentrationRange{a},'AllSem'] ;
%             ForIgor.(identifier) = nanstd(PopData.PsthMinBg_mean{a}{b},[],1)./sqrt(sum(~isnan(PopData.PsthMinBg_mean{a}{b}),1)) ;        
% 
%             identifier = ['vData','LogPulse',num2str(abs(log10(PopData.ConcentrationRange(b)))),'Bg',PopData.BgConcentrationRange{a},'AllSem'] ;
%             ForIgor.(identifier) = nanstd(PopData.vData_mean{a}{b},[],1)./sqrt(sum(~isnan(PopData.vData_mean{a}{b}),1)) ;
% 
%             identifier = ['vDataMinBg','LogPulse',num2str(abs(log10(PopData.ConcentrationRange(b)))),'Bg',PopData.BgConcentrationRange{a},'AllSem'] ;
%             ForIgor.(identifier) = nanstd(PopData.vDataMinBg_mean{a}{b},[],1)./sqrt(sum(~isnan(PopData.vDataMinBg_mean{a}{b}),1)) ;   
%     end
% end
% 
% 
...
%                 identifier = ['SRminBgMean','LogPulse',num2str(abs(log10(PopData.ConcentrationRange(b)))),'Bg',PopData.BgConcentrationRange{a},'AllCnt'] ;
%                 ForIgor.(identifier) = PopData.SRpulseMinBg_mean_paired{a}{b}(1,:) ;
% 
%                 identifier = ['SRminBgSem','LogPulse',num2str(abs(log10(PopData.ConcentrationRange(b)))),'Bg',PopData.BgConcentrationRange{a},'AllCnt'] ;
%                 ForIgor.(identifier) = PopData.SRpulseMinBg_sem_paired{a}{b}(1,:) ;
% 
%                 identifier = ['VminBgMean','LogPulse',num2str(abs(log10(PopData.ConcentrationRange(b)))),'Bg',PopData.BgConcentrationRange{a},'AllCnt'] ;
%                 ForIgor.(identifier) = PopData.VpulseMinBg_mean_paired{a}{b}(1,:) ; 
% 
%                 identifier = ['VminBgSem','LogPulse',num2str(abs(log10(PopData.ConcentrationRange(b)))),'Bg',PopData.BgConcentrationRange{a},'AllCnt'] ;
%                 ForIgor.(identifier) = PopData.VpulseMinBg_sem_paired{a}{b}(1,:) ; 
% 
% 
%                 identifier = ['SRminBgMean','LogPulse',num2str(abs(log10(PopData.ConcentrationRange(b)))),'Bg',PopData.BgConcentrationRange{a},'AllBg'] ;
%                 ForIgor.(identifier) = PopData.SRpulseMinBg_mean_paired{a}{b}(2,:) ;
% 
%                 identifier = ['SRminBgSem','LogPulse',num2str(abs(log10(PopData.ConcentrationRange(b)))),'Bg',PopData.BgConcentrationRange{a},'AllBg'] ;
%                 ForIgor.(identifier) = PopData.SRpulseMinBg_sem_paired{a}{b}(2,:) ;
% 
%                 identifier = ['VminBgMean','LogPulse',num2str(abs(log10(PopData.ConcentrationRange(b)))),'Bg',PopData.BgConcentrationRange{a},'AllBg'] ;
%                 ForIgor.(identifier) = PopData.VpulseMinBg_mean_paired{a}{b}(2,:) ; 
% 
%                 identifier = ['VminBgSem','LogPulse',num2str(abs(log10(PopData.ConcentrationRange(b)))),'Bg',PopData.BgConcentrationRange{a},'AllBg'] ;
%                 ForIgor.(identifier) = PopData.VpulseMinBg_sem_paired{a}{b}(2,:) ;
%             end
%         end   
%     end
% end
