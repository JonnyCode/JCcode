function ForIgor = adaptationAnalysisC(Input,A,AnalysisFlag)

% adaptation of "adaptationAnalysisB" 
% adds analysis of pulse response kinetics
...
% pulse response kinetics analysis
N = 1 ;
for a = 1:length(PopData.BgConcentrationRange)-1 ; % for each background (but 'wash')
    for b = 1:length(PopData.ConcentrationRange) ; % for each pulse concentration
        for c = 1:size(PopData.NumTrials{a},1) ; % for each cell
            if PopData.NumTrials{a}(c,b)>0 ; % if there is data in this cell at this background
                PulseMat(N,:) = PopData.PsthMinBg_mean{a}{b}(c,150000:165000) ;
                PulseMatDec(N,:) = DecimateWave(PulseMat(N,:),10) ;
                PulseMatDecNorm(N,:) = PulseMatDec(N,:)/max(abs(PulseMatDec(N,:))) ;
                N=N+1 ;
            end
        end
    end 
...
PulseMatDecNorm_Cov = cov(PulseMatDecNorm) ; %PCA on normalized pulse responses
[v,d] = eig(PulseMatDecNorm_Cov) ;  
...
for a = 1:length(PopData.BgConcentrationRange)-1 ; % for each background (but 'wash')
    for b = 1:length(PopData.ConcentrationRange) ; % for each pulse concentration
        for c = 1:size(PopData.NumTrials{a},1) ; % for each cell
            if PopData.NumTrials{a}(c,b)>0 ; % if there is data in this cell at this background
                for d = 1:NumPCs ;
                    Proj{a}{b}(c,d) = PulseMatDecNorm(N,:)*v(:,end+1-d) ;
                end
                N=N+1 ;
            end
        end
    ...
    for b = 1:length(PopData.ConcentrationRange) ; % for each pulse concentration
        for c = 1:size(PopData.NumTrials{a},1) ; % for each cell
            if PopData.NumTrials{a}(c,b)>0 ; % if there is data in this cell at this background
                for d = 1:NumPCs ;
                    subplot(1,NumPCs,d)
                    plot(Proj{1}{b}(c,d),Proj{a}{b}(c,d),'*','Color',colorMat{b}(a,:))
                    hold on
                end
            end
        end
...
if strcmp(AnalysisFlag,'ORNdata')
    ...
    ForIgor.OrnDivCon = PopData.SRpulseMeanMinBg_mean_DivCon_AdbWMean ;
    ForIgor.OrnDivCon_sem =PopData.SRpulseMeanMinBg_mean_DivCon_AdbSem ;
elseif strcmp(AnalysisFlag,'PNdata')
