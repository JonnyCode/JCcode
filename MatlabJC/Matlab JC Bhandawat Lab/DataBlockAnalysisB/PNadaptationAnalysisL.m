function ForIgor = PNadaptationAnalysisL(Input,A)

% adaptation of "PNadaptationAnalysisK" 
% normalizes each background response by control in that cell using subtraction
...
for DB = 1:length(A) ; % for each cell
    ...
    % normalized to control parameters
    for a = 1:NumBackgrounds ; % for each background

        SRbgMeanMinRest_ApMean_MinCon(a) = SRbgMeanMinRest_ApMean(a) - SRbgMeanMinRest_ApMean(1) ;
        SRbgMeanMinRest_ApSem_MinCon(a) = sqrt(SRbgMeanMinRest_ApSem(a)^2 + SRbgMeanMinRest_ApSem(1)^2) ; % uncertanties add in quadrature

        SRbgStd_ApMean_MinCon(a) = SRbgStd_ApMean(a) - SRbgStd_ApMean(1) ;
        SRbgStd_ApSem_MinCon(a) = sqrt(SRbgStd_ApSem(a)^2 + SRbgStd_ApSem(1)^2) ; 

        for b = 1:NumConcentrations ; % for concentration
            if NumTrials(a,b)>0 ;
                SRpulseMeanMinBg_mean_MinCon(a,b) = SRpulseMeanMinBg_mean(a,b) - SRpulseMeanMinBg_mean(1,b) ;
                SRpulseMeanMinBg_sem_MinCon(a,b) = sqrt(SRpulseMeanMinBg_sem(a,b)^2 + SRpulseMeanMinBg_sem(1,b)^2) ; 
                
                SRpulseMeanMinBg_std_MinCon(a,b) = SRpulseMeanMinBg_std(a,b) - SRpulseMeanMinBg_std(1,b) ;
                SRpulseMeanMinBg_std_unc_MinCon(a,b) = sqrt(SRpulseMeanMinBg_std_unc(a,b)^2 + SRpulseMeanMinBg_std_unc(1,b)^2) ;                  
            else
                SRpulseMeanMinBg_mean_MinCon(a,b) = nan ;
                SRpulseMeanMinBg_sem_MinCon(a,b) = nan ; 
                
                SRpulseMeanMinBg_std_MinCon(a,b) = nan ;
                SRpulseMeanMinBg_std_unc_MinCon(a,b) = nan ;       
            end
        end
    end
...
% 
    ...
    if DB==1 ;
        for a=1:length(PopData.BgConcentrationRange) ;
            PopData.SRpulseMeanMinBg_mean_MinCon{a} = nan(length(A),length(PopData.ConcentrationRange)) ;
            PopData.SRpulseMeanMinBg_sem_MinCon{a} = nan(length(A),length(PopData.ConcentrationRange)) ;
            PopData.SRpulseMeanMinBg_std_MinCon{a} = nan(length(A),length(PopData.ConcentrationRange)) ;
            PopData.SRpulseMeanMinBg_std_unc_MinCon{a} = nan(length(A),length(PopData.ConcentrationRange)) ;
        end
    end
    for a = 1:NumBackgrounds ; % for each background
        PopData.SRpulseMeanMinBg_mean_MinCon{PopBgi(a)}(DB,PopPulsei) = SRpulseMeanMinBg_mean_MinCon(a,:) ;
        PopData.SRpulseMeanMinBg_sem_MinCon{PopBgi(a)}(DB,PopPulsei) = SRpulseMeanMinBg_sem_MinCon(a,:) ;
        PopData.SRpulseMeanMinBg_std_MinCon{PopBgi(a)}(DB,PopPulsei) = SRpulseMeanMinBg_std_MinCon(a,:) ;
        PopData.SRpulseMeanMinBg_std_unc_MinCon{PopBgi(a)}(DB,PopPulsei) = SRpulseMeanMinBg_std_unc_MinCon(a,:) ;
    end
    
    % SR background data {bg} (cell)
    if DB==1 ;
        for a=1:length(PopData.BgConcentrationRange) ;
            PopData.SRbgMeanMinRest_ApMean_MinCon{a} = nan(1,length(A)) ;
            PopData.SRbgMeanMinRest_ApSem_MinCon{a} = nan(1,length(A)) ;
            PopData.SRbgStd_ApMean_MinCon{a} = nan(1,length(A)) ;
            PopData.SRbgStd_ApSem_MinCon{a} = nan(1,length(A)) ;
        end
    end
    for a = 1:NumBackgrounds ; % for each background
        PopData.SRbgMeanMinRest_ApMean_MinCon{PopBgi(a)}(DB) = SRbgMeanMinRest_ApMean_MinCon(a) ;
        PopData.SRbgMeanMinRest_ApSem_MinCon{PopBgi(a)}(DB) = SRbgMeanMinRest_ApSem_MinCon(a) ;
        PopData.SRbgStd_ApMean_MinCon{PopBgi(a)}(DB) = SRbgStd_ApMean_MinCon(a) ;
        PopData.SRbgStd_ApSem_MinCon{PopBgi(a)}(DB) = SRbgStd_ApSem_MinCon(a) ;
    end
...
for a = 1:length(PopData.BgConcentrationRange) ; % for each background
    if ~isempty(PopData.SRbgMeanMinRest_ApMean{a}) ; % if there are DB at this background
        PopData.SRbgMeanMinRest_ApMean_MinCon_AdbWMean(a) = nansum(PopData.SRbgMeanMinRest_ApMean_MinCon{a}./PopData.SRbgMeanMinRest_ApSem_MinCon{a})./nansum(1./PopData.SRbgMeanMinRest_ApSem_MinCon{a}) ; % wieghted mean across data blocks
        PopData.SRbgMeanMinRest_ApMean_MinCon_AdbSem(a) = nanstd(PopData.SRbgMeanMinRest_ApMean_MinCon{a})./sqrt(sum(~isnan(PopData.SRbgMeanMinRest_ApMean_MinCon{a}))) ; 
    end
end
...
for a = 1:length(PopData.BgConcentrationRange) ; % for each background
    if ~isempty(PopData.SRbgMeanMinRest_ApMean{a}) ; % if there are DB at this background
        PopData.SRbgStd_ApMean_MinCon_AdbWMean(a) = nansum(PopData.SRbgStd_ApMean_MinCon{a}./PopData.SRbgStd_ApSem_MinCon{a})./nansum(1./PopData.SRbgStd_ApSem_MinCon{a}) ; % wieghted mean across data blocks
        PopData.SRbgStd_ApMean_MinCon_AdbSem(a) = nanstd(PopData.SRbgStd_ApMean_MinCon{a})./sqrt(sum(~isnan(PopData.SRbgStd_ApMean_MinCon{a}))) ; 
    end
end
...
for a = 1:length(PopData.BgConcentrationRange) ; % for each background
    if ~isempty(PopData.SRpulseMeanMinBg_mean{a}) ; % if there are DB at this background
        PopData.SRpulseMeanMinBg_mean_MinCon_AdbWMean(a,:) = nansum(PopData.SRpulseMeanMinBg_mean_MinCon{a}./PopData.SRpulseMeanMinBg_sem_MinCon{a},1)./nansum(1./PopData.SRpulseMeanMinBg_sem_MinCon{a},1) ; % wiehgted mean across db
        PopData.SRpulseMeanMinBg_mean_MinCon_AdbSem(a,:) = nanstd(PopData.SRpulseMeanMinBg_mean_MinCon{a},[],1)./sqrt(sum(~isnan(PopData.SRpulseMeanMinBg_mean_MinCon{a}),1)) ; 
    end
end
...
for a = 1:length(PopData.BgConcentrationRange) ; % for each background
    if ~isempty(PopData.SRpulseMeanMinBg_mean{a}) ; % if there are DB at this background
        PopData.SRpulseMeanMinBg_std_MinCon_AdbWMean(a,:) = nansum(PopData.SRpulseMeanMinBg_std_MinCon{a}./PopData.SRpulseMeanMinBg_std_unc_MinCon{a},1)./nansum(1./PopData.SRpulseMeanMinBg_std_unc_MinCon{a},1) ; % wiehgted mean across db
        PopData.SRpulseMeanMinBg_std_MinCon_AdbSem(a,:) = nanstd(PopData.SRpulseMeanMinBg_std_MinCon{a},[],1)./sqrt(sum(~isnan(PopData.SRpulseMeanMinBg_std_MinCon{a}),1)) ; 
    end
end

% applying normalization factors to mean control data
PopData.SRbgMeanMinRest_ApMean_MinCon_AdbWMean_PlusCon = PopData.SRbgMeanMinRest_ApMean_AdbWMean(1) + PopData.SRbgMeanMinRest_ApMean_MinCon_AdbWMean ;
PopData.SRbgMeanMinRest_ApMean_MinCon_AdbSem_PlusCon = sqrt(PopData.SRbgMeanMinRest_ApMean_AdbSem(1)^2 + PopData.SRbgMeanMinRest_ApMean_MinCon_AdbSem.^2) ;

PopData.SRbgStd_ApMean_MinCon_AdbWMean_PlusCon = PopData.SRbgStd_ApMean_AdbWMean(1) + PopData.SRbgMeanMinRest_ApMean_MinCon_AdbWMean ;
PopData.SRbgMeanMinRest_ApStd_MinCon_AdbSem_PlusCon = sqrt(PopData.SRbgMeanMinRest_ApMean_AdbSem(1)^2 + PopData.SRbgMeanMinRest_ApMean_MinCon_AdbSem.^2) ;

for a = 1:length(PopData.BgConcentrationRange) ; % for each background
    if ~isempty(PopData.SRpulseMeanMinBg_mean{a}) ; % if there are DB at this background
        PopData.SRpulseMeanMinBg_mean_MinCon_AdbWMean_PlusCon(a,:) = PopData.SRpulseMeanMinBg_mean_AdbWMean(1,:) + PopData.SRpulseMeanMinBg_mean_MinCon_AdbWMean(a,:) ;
        PopData.SRpulseMeanMinBg_mean_MinCon_AdbSem_PlusCon(a,:) = sqrt(PopData.SRpulseMeanMinBg_mean_AdbSem(1,:).^2 + PopData.SRpulseMeanMinBg_mean_MinCon_AdbSem(a,:).^2) ;
    
        PopData.SRpulseMeanMinBg_std_MinCon_AdbWMean_PlusCon(a,:) = PopData.SRpulseMeanMinBg_std_AdbWMean(1,:) + PopData.SRpulseMeanMinBg_std_MinCon_AdbWMean(a,:) ;
        PopData.SRpulseMeanMinBg_std_MinCon_AdbSem_PlusCon(a,:) = sqrt(PopData.SRpulseMeanMinBg_std_AdbSem(1,:).^2 + PopData.SRpulseMeanMinBg_std_MinCon_AdbSem(a,:).^2) ;
    end
end
...
for a = 1:length(PopData.BgConcentrationRange)-1 ; % for each background (but 'wash')
    if ~isempty(PopData.SRbgMeanMinRest_ApMean{a}) ; % if there are DB at this background
        for b = 1:length(PopData.SRbgMeanMinRest_ApMean{a}) ; % for each DB at that background
            ...
            subplot(2,1,1)
            errorbar(a,PopData.SRbgMeanMinRest_ApMean_MinCon{a}(b),PopData.SRbgMeanMinRest_ApSem_MinCon{a}(b),PopData.SRbgMeanMinRest_ApSem_MinCon{a}(b),'Color',colorMat{5}(a,:))
            hold on
            plot(a,PopData.SRbgMeanMinRest_ApMean_MinCon_AdbWMean(a),'o','LineWidth',2,'Color',colorMat{5}(a,:))
            
            subplot(2,1,2)
            errorbar(a,PopData.SRbgMeanMinRest_ApMean_MinCon_AdbWMean_PlusCon(a),PopData.SRbgMeanMinRest_ApMean_MinCon_AdbSem_PlusCon(a),PopData.SRbgMeanMinRest_ApMean_MinCon_AdbSem_PlusCon(a),'o','LineWidth',2,'Color',colorMat{5}(a,:))
            hold on
            ylabel('SRbgMeanMinRest')
            xlabel('background')
            set(gca,'Xtick',[1:length(PopData.BgConcentrationRange)-1])
...
for a = 1:length(PopData.BgConcentrationRange)-1 ; % for each background (but 'wash')
    if ~isempty(PopData.SRbgMeanMinRest_ApMean{a}) ; % if there are DB at this background
        for b = 1:length(PopData.SRbgMeanMinRest_ApMean{a}) ; % for each DB at that background
            ...
            subplot(2,1,1)
            errorbar(log10(PopData.ConcentrationRange),PopData.SRpulseMeanMinBg_mean_MinCon{a}(b,:),PopData.SRpulseMeanMinBg_sem_MinCon{a}(b,:),PopData.SRpulseMeanMinBg_sem_MinCon{a}(b,:),'Color',colorMat{5}(a,:))
            hold on
            plot(log10(PopData.ConcentrationRange),PopData.SRpulseMeanMinBg_mean_MinCon_AdbWMean(a,:),'o','LineWidth',2,'Color',colorMat{5}(a,:))
            
            subplot(2,1,2)
            errorbar(log10(PopData.ConcentrationRange),PopData.SRpulseMeanMinBg_mean_MinCon_AdbWMean_PlusCon(a,:),PopData.SRpulseMeanMinBg_mean_MinCon_AdbSem_PlusCon(a,:),PopData.SRpulseMeanMinBg_mean_MinCon_AdbSem_PlusCon(a,:),'LineWidth',2,'Color',colorMat{5}(a,:))
            hold on
            ylabel('SRpulseMeanMinBg')
            xlabel('log(pulse)')
