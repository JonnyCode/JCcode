function ForIgor = KoSpatialTuningAnalyzer(DataBlock, DB, Params)
...
for ds = 1:numDs % for each data set
    ...
    % load data
    dataRunMaster = load_data(DataBlock(DB).BwPath{ds}) ;
    ...
    for uc = 1:length(UniqueCellTypes) ;
        cell_ids{uc}{ds} = get_cell_ids(dataRunMaster, UniqueCellTypes{uc}) ;
    end
