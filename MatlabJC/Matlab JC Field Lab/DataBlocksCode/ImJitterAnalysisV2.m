function ForIgor = ImJitterAnalysisV2(DataBlock, DB, Params)


% edited from "ImJitterAnalysis" to change OLE
...
ImNum = 3 ; %TEMP -SHOULD LOOP THIS EVENTUALLY!
DsPathNum = 2 ;
...
if ~ischar(stimulus.XSquareJitterVector) && stimulus.square_jitter_std>0 ; % if there was a jittered square
    DirectionWieghts = StaMagSquareJitterVector{1}(1,(StaTime>-.3 & StaTime<=0)) ;
else
...
if ~ischar(stimulus.XSquareJitterVector) && stimulus.square_jitter_std>0 ; % if there was a jittered square
    ...
    MagSquareJitterVector = sqrt(XSquareJitterVectorDiff.^2 + YSquareJitterVectorDiff.^2) ; % magnitude
    DirSquareJitterVector = cart2pol(XSquareJitterVectorDiff,YSquareJitterVectorDiff) ; % direction of movement (radians)
    
    for DsType=1:length(DsTypeName) ; % for each DS cell type
        
        xF = mean(StaXSquareJitterVector{DsType}(:,(StaTime>-.3 & StaTime<=0)),1) ; % x filter
        yF = mean(StaYSquareJitterVector{DsType}(:,(StaTime>-.3 & StaTime<=0)),1) ; % y filter
        magF = sqrt(xF.^2+yF.^2) ; % filter mag
        ...
        for s=1:NumStimPnts ; % for each stimulus bin
            ...
            lp(DsType,s) = sum(XSquareJitterVectorDiff(startPnt:s).*xF(end-endOffset:end) +...
                YSquareJitterVectorDiff(startPnt:s).*xF(end-endOffset:end)) ;
        end
        
%         xF = fliplr(mean(StaXSquareJitterVector{DsType},1)) ; % x filter
%         yF = fliplr(mean(StaYSquareJitterVector{DsType},1)) ; % y filter
%         magF = sqrt(xF.^2+yF.^2) ; % filter mag
%         
%         XSquareJitter_lp = conv(XSquareJitterVector,xF,'same') ; % linear prediction of X
%         YSquareJitter_lp = conv(YSquareJitterVector,yF,'same') ; % linear prediction of Y
%         lp(DsType,:) = XSquareJitter_lp + YSquareJitter_lp  ; % linear prediction (dot product)
...
if ~ischar(stimulus.XImageJitterVector) && stimulus.image_jitter_std>0 ; % if there was a jittered image
    ...
    DirImageJitterVectorDeg = DirImageJitterVector*180/pi+180 ;
    ...
    DirImageJitterVectorDeg_wieghted = DirImageJitterVector_wieghted*180/pi+180 ;
    
    for td = 0:20 ; % for each time delay
        %[OleWeights, OlePolarWeights] = OleFinder(DirImageJitterVectorDeg_wieghted(1:size(psthMat,1)-td),psthMat((td+1):end,:),'OQE','Regularize',10) ;
        [OleWeights, OlePolarWeights] = OleFinder(DirImageJitterVectorDeg(1:size(psthMat,1)-td),psthMat((td+1):end,:),'OQE','Regularize',10) ;
        [DirectionEstimate,DirectionEstimateMag] = OleTester(OleWeights,psthMat((td+1):end,:),'OQE');
        Est{td+1} = DirectionEstimate ;
    end
    
    % error
    for td = 0:20 ; % for each time delay
        for s=1:(NumStimPnts-20) ; % for each stim bin
            %EstError{td+1}(s) = acuteAngle(Est{td+1}(s),DirImageJitterVectorDeg_wieghted(s)) ;
            EstError{td+1}(s) = acuteAngle(Est{td+1}(s),DirImageJitterVectorDeg(s)) ;
        end
    end
end
...
if ~ischar(stimulus.XSquareJitterVector) && stimulus.square_jitter_std>0 ; % if there was a jittered square
    ...
    DirSquareJitterVectorDeg = DirSquareJitterVector*180/pi+180 ;

    DirectionWieghts = StaMagSquareJitterVector{1}(1,(StaTime>-.3 & StaTime<=0)) ;
    for s=1:NumStimPnts ; % for each stim bin
        startPnt = max(1,s-length(DirectionWieghts)+1) ;
        DirSquareJitterVector_wieghted(s) = circ_mean(DirSquareJitterVector(startPnt:s)',...
            DirectionWieghts(end-min(s-1,length(DirectionWieghts)-1):end)') ; % % direction as filtered through sta
    end

    DirSquareJitterVectorDeg_wieghted = DirSquareJitterVector_wieghted*180/pi+180 ;
    
    for td = 0:20 ; % for each time delay
        [OleWeights, OlePolarWeights] = OleFinder(DirSquareJitterVectorDeg_wieghted(1:size(psthMat,1)-td),psthMat((td+1):end,:),'OQE','Regularize',10) ;
        %[OleWeights, OlePolarWeights] = OleFinder(DirSquareJitterVectorDeg(1:size(psthMat,1)-td),psthMat((td+1):end,:),'OQE','Regularize',10) ;
        [DirectionEstimate,DirectionEstimateMag] = OleTester(OleWeights,psthMat((td+1):end,:),'OQE');
        Est{td+1} = DirectionEstimate ;
    end
    ...
    for td = 0:20 ; % for each time delay
        for s=1:(NumStimPnts-20) ; % for each stim bin
            EstError{td+1}(s) = acuteAngle(Est{td+1}(s),DirSquareJitterVectorDeg_wieghted(s)) ;
            %EstError{td+1}(s) = acuteAngle(Est{td+1}(s),DirSquareJitterVectorDeg(s)) ;
        end
    end
end
...
figure
for td=1:20 ;
    plot(DirImageJitterVectorDeg_wieghted(1:length(Est{td})),'k')
    hold on
    plot(Est{td},'r')
    hold off
    pause
...
figure
for td=1:20 ;
    plot(td,mean(EstError{td}),'go')
    hold on
end

figure
for td=1:20 ;
    plot(td,mean(EstError{td}),'ko')
    hold on
end
