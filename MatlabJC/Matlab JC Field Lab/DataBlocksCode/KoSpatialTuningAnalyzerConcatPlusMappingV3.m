function ForIgor = KoSpatialTuningAnalyzerConcatPlusMappingV3(DataBlock, DB, Params)

% this function is adapted from 'KoSpatialTuningAnalyzerConcatPlusMapping2'.
%  Changed and eliminated parameter analysis
        ...
        for sp=1:l_sp ; % for each spatial period
            for cntrst=1:l_cntrst ; % for each contrast
                for cells = 1:l_cells ; % for each cell
                    ...
                    psth_varF1_divCntrl{dset}{tp}{cells}(cntrst,sp) = DiffOverSum(psth_varF1{dset}{tp}{cntrst}(cells,sp),psth_varF1{1}{tp}{cntrst}(cells,sp)) ; % condition,control
                    psth_varF2_divCntrl{dset}{tp}{cells}(cntrst,sp) = DiffOverSum(psth_varF2{dset}{tp}{cntrst}(cells,sp),psth_varF2{1}{tp}{cntrst}(cells,sp)) ; % condition,control
...
% drug changes averaged across contrasts
for cells = 1:l_cells ; % for each cell
    psth_var_divCntrl_cMean(cells,:) =  nanmean(psth_var_divCntrl{2}{1}{cells}); 
    psth_range_divCntrl_cMean(cells,:) = nanmean(psth_range_divCntrl{2}{1}{cells}) ;
    psth_mean_divCntrl_cMean(cells,:) =  nanmean(psth_mean_divCntrl{2}{1}{cells}); 
    psth_duty_divCntrl_cMean(cells,:) = nanmean(psth_duty_divCntrl{2}{1}{cells}) ;
    psth_peakDivMean_divCntrl_cMean(cells,:) = nanmean(psth_peakDivMean_divCntrl{2}{1}{cells}) ;
    psth_cct_deltaCntrl_cMean(cells,:) = nanmean(psth_cct_deltaCntrl{2}{1}{cells}) ; 
end

% drug changes averaged across cntrst and low frequencies stim
for cells = 1:l_cells ; % for each cell
    psth_var_divCntrl_LfMean(cells) =  nanmean(psth_var_divCntrl_cMean(cells,:)); 
    psth_range_divCntrl_LfMean(cells) = nanmean(psth_range_divCntrl_cMean(cells,:)) ;
    psth_mean_divCntrl_LfMean(cells) =  nanmean(psth_mean_divCntrl_cMean(cells,:)); 
    psth_duty_divCntrl_LfMean(cells) = nanmean(psth_duty_divCntrl_cMean(cells,:)) ;
    psth_peakDivMean_divCntrl_LfMean(cells) = nanmean(psth_peakDivMean_divCntrl_cMean(cells,:)) ;
    psth_cct_deltaCntrl_LfMean(cells) = nanmean(psth_cct_deltaCntrl_cMean(cells,:)) ; 
end

% histogram for cells that had strong drug effects
[psth_var_divCntrl_LfMean_hist,psth_var_divCntrl_LfMean_histX] =  hist(psth_var_divCntrl_LfMean(MseRelDrug_DivMseRelWashLfMean>MseRelDrugDivWashThreshold),[-1.1:.1:1.1]) ; 
[psth_range_divCntrl_LfMean_hist,psth_range_divCntrl_LfMean_histX] = hist(psth_range_divCntrl_LfMean(MseRelDrug_DivMseRelWashLfMean>MseRelDrugDivWashThreshold),[-1.1:.1:1.1]) ;
[psth_mean_divCntrl_LfMean_hist,psth_mean_divCntrl_LfMean_histX] =  hist(psth_mean_divCntrl_LfMean(MseRelDrug_DivMseRelWashLfMean>MseRelDrugDivWashThreshold),[-1.1:.1:1.1]) ;
[psth_duty_divCntrl_LfMean_hist,psth_duty_divCntrl_LfMean_histX] = hist(psth_duty_divCntrl_LfMean(MseRelDrug_DivMseRelWashLfMean>MseRelDrugDivWashThreshold),[-1.1:.1:1.1]) ;
[psth_peakDivMean_divCntrl_LfMean_hist,psth_peakDivMean_divCntrl_LfMean_histX] = hist(psth_peakDivMean_divCntrl_LfMean(MseRelDrug_DivMseRelWashLfMean>MseRelDrugDivWashThreshold),[-1.1:.1:1.1]) ;
    ...
    for tp=1:l_tp ; % for each temporal period
        for cells = 1:l_cells ; % for each cell
            ...
            for cntrst=1:l_cntrst ; % for each contrast
                ...
                vf1 = vf1 + psth_varF1{dset}{tp}{cntrst}(cells,:) ;
                vf2 = vf2 + psth_varF2{dset}{tp}{cntrst}(cells,:) ;
                ...
                vf2of1 = vf2of1 + psth_varF2F1ratio{dset}{tp}{cntrst}(cells) ;
            end
            psth_var_cMean{dset}{tp}(cells,:) = va/l_cntrst ;
            psth_varF1_cMean{dset}{tp}(cells,:) = vf1/l_cntrst ;
            psth_varF2_cMean{dset}{tp}(cells,:) = vf2/l_cntrst ;
            ...
            psth_varF2F1ratio_cMean{dset}{tp}(cells) = vf2of1/l_cntrst ;
        end
    end
end
...
for uc = 1:length(UniqueCellTypes) ; % for each cell type
    cells = find(MseRelDrug_DivMseRelWashLfMean(cell_i{uc})>MseRelDrugDivWashThreshold) ;

    psth_var_divCntrl_cMean_AllCells(uc,:) = nanmean(psth_var_divCntrl_cMean(cell_i{uc},:),1) ; % all cells of type uc
    psth_var_divCntrl_cMean_ThreshCells(uc,:) = nanmean(psth_var_divCntrl_cMean(cell_i{uc}(cells),:),1) ; % cells of type uc that pass mse threshold
    
    psth_var_cgain_DivCntrl_AllCells(uc,:) = nanmean(psth_var_cgain_DivCntrl{2}(cell_i{uc},:),1) ; % all cells of type uc
    psth_var_cgain_DivCntrl_ThreshCells(uc,:) = nanmean(psth_var_cgain_DivCntrl{2}(cell_i{uc}(cells),:),1) ;
    
    psth_mean_divCntrl_cMean_AllCells(uc,:) = nanmean(psth_mean_divCntrl_cMean(cell_i{uc},:),1) ; % all cells of type uc
    psth_mean_divCntrl_cMean_ThreshCells(uc,:) = nanmean(psth_mean_divCntrl_cMean(cell_i{uc}(cells),:),1) ;
    
    psth_duty_divCntrl_cMean_AllCells(uc,:) = nanmean(psth_duty_divCntrl_cMean(cell_i{uc},:),1) ; % all cells of type uc
    psth_duty_divCntrl_cMean_ThreshCells(uc,:) = nanmean(psth_duty_divCntrl_cMean(cell_i{uc}(cells),:),1) ;
    
    psth_cct_deltaCntrl_cMean_AllCells(uc,:) = nanmean(psth_cct_deltaCntrl_cMean(cell_i{uc},:),1) ; % all cells of type uc
    psth_cct_deltaCntrl_cMean_ThreshCells(uc,:) = nanmean(psth_cct_deltaCntrl_cMean(cell_i{uc}(cells),:),1) ;
    
    psth_varF2F1ratio_cMean_AllCells(uc) = nanmean(psth_varF2F1ratio_cMean{1}{1}(cell_i{uc}(cells))) ;
end
...
sp = 11 ; % spatial frequency default
...
% LfMean parameter comparisons for cells with (black) and without (cyan) relatively large drug changes
figure
for uc = 1:length(UniqueCellTypes) ;
    lc = length(cell_i{uc}) ; % number of cell of that type
    for cells=1:lc ; % for each cell of this type
        ...
        subplot(6,2,1)
        plot(uc,psth_var_divCntrl_LfMean(cell_i{uc}(cells)),[PlotColor,SignPnt])
        hold on
        xlabel('cell type')
        ylabel('Variance drug/cntrl')

        subplot(6,2,3)
        plot(uc,psth_range_divCntrl_LfMean(cell_i{uc}(cells)),[PlotColor,SignPnt])
        hold on
        xlabel('cell type')
        ylabel('Range drug/cntrl')

        subplot(6,2,5)
        plot(uc,psth_mean_divCntrl_LfMean(cell_i{uc}(cells)),[PlotColor,SignPnt])
        hold on
        xlabel('cell type')
        ylabel('Mean drug/cntrl')

        subplot(6,2,7)
        plot(uc,psth_duty_divCntrl_LfMean(cell_i{uc}(cells)),[PlotColor,SignPnt])
        hold on
        xlabel('cell type')
        ylabel('Duty drug/cntrl')

        subplot(6,2,9)
        plot(uc,psth_peakDivMean_divCntrl_LfMean(cell_i{uc}(cells)),[PlotColor,SignPnt])
        hold on
        xlabel('cell type')
        ylabel('peak/mean drug/cntrl')

        subplot(6,2,11)
        plot(uc,psth_cct_deltaCntrl_LfMean(cell_i{uc}(cells)),[PlotColor,SignPnt])
        hold on
        xlabel('cell type')
...
% spatial tuning functions averaged across contrasts for each cell type (F1)
for uc = 1:length(UniqueCellTypes) ; % for each cell type
    figure
    set(gcf,'name',UniqueCellTypes{uc})
    lc = length(cell_i{uc}) ; % number of cell of that type
    for cells=1:lc ; % for each cell of this type
        subplot(ceil(lc/3),3,cells) 
        for dset=1:3 ; 
            plot(Spatial_frequency,psth_varF1_cMean{dset}{1}(cell_i{uc}(cells),:),'color',Color_list{dset})
            hold on
            plot(Spatial_frequency,psth_varF2_cMean{dset}{1}(cell_i{uc}(cells),:),'--','color',Color_list{dset})
        end
        set(gca,'Xscale','log')
        title(num2str(cell_i{uc}(cells)))
    end
...
for uc = 1:length(UniqueCellTypes) ; % for each cell type
    ...
    for cells=1:lc ; % for each cell of this type
        ...
        subplot(5,1,1) 
        plot(Spatial_frequency,psth_var_divCntrl_cMean(cell_i{uc}(cells),:), PlotColor)
        hold on
        ...
        subplot(5,1,3) 
        plot(Spatial_frequency,psth_mean_divCntrl_cMean(cell_i{uc}(cells),:), PlotColor)
        hold on

        subplot(5,1,4) 
        plot(Spatial_frequency,psth_duty_divCntrl_cMean(cell_i{uc}(cells),:), PlotColor)
        hold on
        
        subplot(5,1,5) 
        plot(Spatial_frequency,psth_cct_deltaCntrl_cMean(cell_i{uc}(cells),:), PlotColor)
        hold on
    end
    
    subplot(5,1,1) 
    plot(Spatial_frequency,psth_var_divCntrl_cMean_AllCells(uc,:),'b','linewidth',2)
    plot(Spatial_frequency,psth_var_divCntrl_cMean_ThreshCells(uc,:),'r','linewidth',2)
    ...
    subplot(5,1,2) 
    plot(Spatial_frequency,psth_var_cgain_DivCntrl_AllCells(uc,:),'b','linewidth',2)
    plot(Spatial_frequency,psth_var_cgain_DivCntrl_ThreshCells(uc,:),'r','linewidth',2)
    plot([Spatial_frequency(1),Spatial_frequency(end)],[0,0],'r:')
    set(gca,'Xscale','log')
    axis tight
    ...
    subplot(5,1,3) 
    plot(Spatial_frequency,psth_mean_divCntrl_cMean_AllCells(uc,:),'b','linewidth',2)
    plot(Spatial_frequency,psth_mean_divCntrl_cMean_ThreshCells(uc,:),'r','linewidth',2)
    plot([Spatial_frequency(1),Spatial_frequency(end)],[0,0],'r:')
    set(gca,'Xscale','log')
    axis tight
    ylabel('mean drug/cntrl')
    ...
    subplot(5,1,4) 
    plot(Spatial_frequency,psth_duty_divCntrl_cMean_AllCells(uc,:),'b','linewidth',2)
    plot(Spatial_frequency,psth_duty_divCntrl_cMean_ThreshCells(uc,:),'r','linewidth',2)
    plot([Spatial_frequency(1),Spatial_frequency(end)],[0,0],'r:')
    set(gca,'Xscale','log')
    axis tight
    ylabel('duty drug/cntrl')
    ...
    subplot(5,1,5) 
    plot(Spatial_frequency,psth_cct_deltaCntrl_cMean_AllCells(uc,:),'b','linewidth',2)
    plot(Spatial_frequency,psth_cct_deltaCntrl_cMean_ThreshCells(uc,:),'r','linewidth',2)
    plot([Spatial_frequency(1),Spatial_frequency(end)],[0,0],'r:')
    set(gca,'Xscale','log')
    axis tight
    ylabel('cc shift')
...
for uc = 1:length(UniqueCellTypes) ; % for each cell type
    subplot(6,1,1) 
    plot(Spatial_frequency,nanmean(psth_var_divCntrl_cMean(cell_i{uc},:)),'linewidth',2)
    hold on
    ...
    subplot(6,1,2) 
    plot(Spatial_frequency,nanmean(psth_range_divCntrl_cMean(cell_i{uc},:)),'linewidth',2)
    hold on
    plot([Spatial_frequency(1),Spatial_frequency(end)],[0,0],'g:')
    set(gca,'Xscale','log')
    axis tight
    ...
    subplot(6,1,3) 
    plot(Spatial_frequency,nanmean(psth_mean_divCntrl_cMean(cell_i{uc},:)),'linewidth',2)
    hold on
    plot([Spatial_frequency(1),Spatial_frequency(end)],[0,0],'g:')
    set(gca,'Xscale','log')
    axis tight
    ...
    subplot(6,1,4) 
    plot(Spatial_frequency,nanmean(psth_duty_divCntrl_cMean(cell_i{uc},:)),'linewidth',2)
    hold on
    plot([Spatial_frequency(1),Spatial_frequency(end)],[0,0],'g:')
    set(gca,'Xscale','log')
    axis tight
    ...
    subplot(6,1,5) 
    plot(Spatial_frequency,nanmean(psth_peakDivMean_divCntrl_cMean(cell_i{uc},:)),'linewidth',2)
    hold on
    plot([Spatial_frequency(1),Spatial_frequency(end)],[0,0],'g:')
    set(gca,'Xscale','log')
    axis tight
    ...
    subplot(6,1,6) 
    plot(Spatial_frequency,nanmean(psth_cct_deltaCntrl_cMean(cell_i{uc},:)),'linewidth',2)
    hold on
    plot([Spatial_frequency(1),Spatial_frequency(end)],[0,0],'g:')
    set(gca,'Xscale','log')
    axis tight
...
% figures of variance change for illustration
...
for uc = 1:length(UniqueCellTypes) ; % for each cell type
    ...
    coord_tform = coordinate_transform(dataRunMaster,'sta'); 
    Mi = get_cell_indices(dataRunMaster,UniqueCellTypes{uc}) ;
    for c = 1:length(Mi) ; % for each cell
        ctr = dataRunMaster.stas.fits{Mi(c)}.mean ;
        rad = dataRunMaster.stas.fits{Mi(c)}.sd ;
        angle = dataRunMaster.stas.fits{Mi(c)}.angle ;
        [X,Y] = drawEllipse([ctr rad angle]) ;
        if ~any(isnan([X,Y])) ;
            [X,Y] = tformfwd(coord_tform, X, Y) ;
            if isempty(cell_list_map{Mi(c)})    
                plot(X,Y,'k')
            else
                plot(X,Y,'r')
            end
    ...
    title(['N=',num2str(length(cell_ids{uc})),'/',num2str(length(Mi))])
    ...
    if cell_ids{uc}>0 ;
        ...
        errorbar(Spatial_frequency,mean(psth_var_cMean{1}{1}(cell_i{uc},:),1),std(psth_var_cMean{1}{1}(cell_i{uc},:),[],1)/Nsqrt,std(psth_var_cMean{1}{1}(cell_i{uc},:),[],1)/Nsqrt,'color',Color_list{1})
        hold on
        errorbar(Spatial_frequency,mean(psth_var_cMean{2}{1}(cell_i{uc},:),1),std(psth_var_cMean{2}{1}(cell_i{uc},:),[],1)/Nsqrt,std(psth_var_cMean{2}{1}(cell_i{uc},:),[],1)/Nsqrt,'color',Color_list{2})
        errorbar(Spatial_frequency,mean(psth_var_cMean{3}{1}(cell_i{uc},:),1),std(psth_var_cMean{3}{1}(cell_i{uc},:),[],1)/Nsqrt,std(psth_var_cMean{3}{1}(cell_i{uc},:),[],1)/Nsqrt,'color',Color_list{3})
        set(gca,'Xscale','log')
        axis tight
        ylabel('var (sp/s)^2')
        xlabel('Sf (c/um)')
        ...
        plot(Spatial_frequency,psth_var_divCntrl_cMean_AllCells(uc,:),'c')
        hold on
        plot(Spatial_frequency,psth_var_divCntrl_cMean_ThreshCells(uc,:),'k')
        plot([Spatial_frequency(1),Spatial_frequency(end)],[0,0],'g--')
        set(gca,'Xscale','log')
        axis tight
        set(gca,'ylim',[-1,1])
        ...
        plot(Spatial_frequency,psth_mean_divCntrl_cMean_AllCells(uc,:),'c')
        hold on
        plot(Spatial_frequency,psth_mean_divCntrl_cMean_ThreshCells(uc,:),'k')
        plot([Spatial_frequency(1),Spatial_frequency(end)],[0,0],'g--')
        set(gca,'Xscale','log')
        axis tight
        set(gca,'ylim',[-1,1])
        ...
        plot(Spatial_frequency,psth_duty_divCntrl_cMean_AllCells(uc,:),'c')
        hold on
        plot(Spatial_frequency,psth_duty_divCntrl_cMean_ThreshCells(uc,:),'k')
        plot([Spatial_frequency(1),Spatial_frequency(end)],[0,0],'g--')
        set(gca,'Xscale','log')
        axis tight
        set(gca,'ylim',[-1,1])
