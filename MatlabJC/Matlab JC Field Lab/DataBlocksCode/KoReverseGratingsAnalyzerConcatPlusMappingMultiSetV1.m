function ForIgor = KoReverseGratingsAnalyzerConcatPlusMappingMultiSetV1(DataBlock, DB, Params)

% this function is adapted from
% 'KoReverseGratingsAnalyzerConcatPlusMappingV1' to be used for contrast
% reversing gratings with more than 3 conditions to compare
...
for dset=2:length(DataBlock(DB).DgPath) ;
    for tp=1:l_tp ; % for each temporal period
         psth_varF1_phsMean_DivCntrl{dset}{tp} = psth_varF1_phsMean{dset}{tp}./psth_varF1_phsMean{1}{tp} ;
         psth_varF2_phsMean_DivCntrl{dset}{tp} = psth_varF2_phsMean{dset}{tp}./psth_varF2_phsMean{1}{tp} ;
         psth_var_phsMean_DivCntrl{dset}{tp} = psth_var_phsMean{dset}{tp}./psth_var_phsMean{1}{tp} ;
    end
end
        ...
        for dset=1:length(DataBlock(DB).DgPath) ;
            ...
            if lc>0 ; % if there are identified cells of this type
                for cells=1:lc ; % for each cell of this type  
                    tempMat(cells,:) = psth_var_phsMean{dset}{tp}(cell_i{uc}(cells),:) ;
                    tempMatF1(cells,:) = psth_varF1_phsMean{dset}{tp}(cell_i{uc}(cells),:) ;
                    tempMatF2(cells,:) = psth_varF2_phsMean{dset}{tp}(cell_i{uc}(cells),:) ;            
                end
                psth_var_phsMean_CellMean{dset}{tp}(uc,:) = mean(tempMat,1) ;
                psth_var_phsMean_CellSem{dset}{tp}(uc,:) = std(tempMat,[],1)/sqrt(lc) ; 

                psth_varF1_phsMean_CellMean{dset}{tp}(uc,:) = mean(tempMatF1,1) ;
                psth_varF1_phsMean_CellSem{dset}{tp}(uc,:) = std(tempMatF1,[],1)/sqrt(lc) ;

                psth_varF2_phsMean_CellMean{dset}{tp}(uc,:) = mean(tempMatF2,1) ;
                psth_varF2_phsMean_CellSem{dset}{tp}(uc,:) = std(tempMatF2,[],1)/sqrt(lc) ;
            end      
        end
    end
end
...
while cells <=l_cells ; % for each cell
    ...
    for dset=1:length(DataBlock(DB).RgPath) ;
        for sp=1:l_sp ; % for each spatial period 
            for phs=1:l_phs ; % for each contrast
                ...
                plot(psth{dset}{tp}{phs}{sp}(cells,:),'color',Color_list{dset})
                axis tight
                hold on
                title(['']) ;
            end
    ...
    for dset=1:length(DataBlock(DB).RgPath) ;
        ...
        plot(log(Spatial_frequency),psth_varF1_phsMean{dset}{tp}(cells,:),'color',Color_list{dset})
        hold on
        plot(log(Spatial_frequency),psth_varF2_phsMean{dset}{tp}(cells,:),'color',Color_list{dset},'LineStyle','--')
        ...
        plot(log(Spatial_frequency),psth_var_phsMean{dset}{tp}(cells,:),'color',Color_list{dset})
        xlabel('log 2 spatial f')
        ylabel('psth variance')
        title(num2str(dataRun_cell_ids(cells)))
        hold on
        ...
        plot(log(Spatial_frequency),psth_varF2_phsMean{dset}{tp}(cells,:),'color',Color_list{dset})
        xlabel('log 2 spatial f')
...
for tp=1:l_tp ;
    ...
    for uc = 1:length(UniqueCellTypes) ; % for each cell type
        ...
        for dset=1:length(DataBlock(DB).DgPath) ;       
%             errorbar(Spatial_frequency,psth_var_phsMean_CellMean{dset}{tp}(uc,:),...
%                 psth_var_phsMean_CellSem{dset}{tp}(uc,:),'color',Color_list{dset}) ;
              errorbar(Spatial_frequency,psth_varF2_phsMean_CellMean{dset}{tp}(uc,:),...
                psth_varF2_phsMean_CellSem{dset}{tp}(uc,:),'color',Color_list{dset}) ;
            hold on
        end
        set(gca,'xscale','log')
        set(gca,'yscale','log')
...
for tp=1:l_tp ;
    ...
    for uc = 1:length(UniqueCellTypes) ; % for each cell type
        ...
        for cells=1:lc ; % for each cell of this type
            for dset=2:length(DataBlock(DB).DgPath) ;
                %plot(Spatial_frequency,psth_var_phsMean_DivCntrl{dset}{tp}(cell_i{uc}(cells),:),'color',Color_list{dset})
                plot(Spatial_frequency,psth_varF2_phsMean_DivCntrl{dset}{tp}(cell_i{uc}(cells),:),'color',Color_list{dset})
            end
