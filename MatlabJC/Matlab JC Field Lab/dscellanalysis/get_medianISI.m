function[NumSpikesCell, StimComb] = get_medianISI(datarun)
...
for i = 1:length(datarun.cell_ids)
    ...
    ISInew = ISInew';
    for k = 1:length(datarun.stimulus.combinations)
        NumSpikesCell(i,k) = median(nonzeros(ISInew(:,k)));
    end
...
%         
%     group = datarun.stimulus.trial_list';
%     group = repmat(group, 1, length(ISI));
%     h = gscatter(group, ISI)
%     get(h, 'ydata')
%         
%     NumSpikesCell(i,:) = max(grpstats(ISI,datarun.stimulus.trial_list,'cat')');
