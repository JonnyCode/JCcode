...
%   axis_range      []        for user to specify x & y axis range on plots
%   first_tic       1         first y-tic will start here
%
% 2013-04 sravi added figure appearance changes: ticks, axes spacing etc
% 2014-01 sravi included axis parameters 
% 2015 JC added first_tic option
% 2018 JC fixed issue when plotting raster with two spikes
% 2018 JC added dots_flag option for speed
...
p.addParamValue('dots_flag', false, @islogical) ;
...
if ~params.dots_flag ; % if not ploting dots
    for i = params.first_trial:num_trials
        ...
        if params.plot
            if ~isempty(ith_trial_times)
                line([[ith_trial_times(1);ith_trial_times], [ith_trial_times(1);ith_trial_times]], [trial_plot_line-1+params.line_space, trial_plot_line-params.line_space],'color', params.tic_color, 'LineWidth', params.tic_thickness)
    ...
    for i = params.first_trial:num_trials
        ...
        X = [X;ith_trial_times] ;
        Y = [Y;ones(length(ith_trial_times),1)*i] ;

        % store spike times
        spikes_by_trials{i} = ith_trial_times;
    end

    Y = Y+params.first_tic-1 ;

    if params.plot
        plot(X, Y,'.','color', params.tic_color, 'MarkerSize', params.tic_thickness)
        axis(params.axis_range);
        set(gca,'Box', 'off', 'TickDir','out', 'TickLength', [.02 .02],'XColor',[.1 .1 .1],'YColor',[.1 .1 .1], 'LineWidth', 1);
    end
end
