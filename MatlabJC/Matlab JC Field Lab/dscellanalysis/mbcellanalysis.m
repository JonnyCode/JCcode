function[ds_struct] = mbcellanalysis(NumSpikesCell, StimComb, datarun)

%Calculates vector averages and DS index for each bar_width and speed of moving bar for each cell   
...
%Outputs: rho: N temporal periods x M spatial periods where each cell has normalized spike numbers in each direction 
  ...
  %Revised by Xiaoyang Yao @ 07-01-2015
          
          

% barwidth = unique(cell2mat(StimComb(:,1)));
barwidth = datarun.stimulus.params.BAR_WIDTH;
mag = cell(length(unique(cell2mat(StimComb(:,2)))), length(unique(cell2mat(StimComb(:,1)))), size(unique(cell2mat(StimComb(:,4)), 'rows'), 1));
MAG = cell(length(unique(cell2mat(StimComb(:,2)))), length(unique(cell2mat(StimComb(:,1)))), size(unique(cell2mat(StimComb(:,4)), 'rows'), 1));
dsindex = cell(length(unique(cell2mat(StimComb(:,2)))), length(unique(cell2mat(StimComb(:,1)))), size(unique(cell2mat(StimComb(:,4)), 'rows'), 1));
angle = cell(length(unique(cell2mat(StimComb(:,2)))), length(unique(cell2mat(StimComb(:,1)))), size(unique(cell2mat(StimComb(:,4)), 'rows'), 1));
rho = cell(length(unique(cell2mat(StimComb(:,2)))), length(unique(cell2mat(StimComb(:,1)))), size(unique(cell2mat(StimComb(:,4)), 'rows'), 1));
RHO = cell(length(unique(cell2mat(StimComb(:,2)))), length(unique(cell2mat(StimComb(:,1)))), size(unique(cell2mat(StimComb(:,4)), 'rows'), 1));
theta = cell(length(unique(cell2mat(StimComb(:,2)))), length(unique(cell2mat(StimComb(:,1)))), size(unique(cell2mat(StimComb(:,4)), 'rows'), 1));
U = cell(length(unique(cell2mat(StimComb(:,2)))), length(unique(cell2mat(StimComb(:,1)))), size(unique(cell2mat(StimComb(:,4)), 'rows'), 1));
V = cell(length(unique(cell2mat(StimComb(:,2)))), length(unique(cell2mat(StimComb(:,1)))), size(unique(cell2mat(StimComb(:,4)), 'rows'), 1));

for i = 1:length(barwidth)
    
    inn = ismember(cell2mat(StimComb(:,1)), barwidth(i))';
...
polar_plots_all(U1,V1,num_t, mag1);
...
ds_struct = v2struct(mag, MAG, dsindex, magmax, magave, angle, rho, RHO, theta, num_t, num_c, U, V);
end