...
%add the path with the lasso scripts
addpath('lasso');
addpath('lasso/sub');
...
train_set = permutor(1:ceil(trainfrac*Ntrials)); %use 80% for training (these percentages can obv. be changed)
test_set = permutor(ceil(trainfrac*Ntrials)+1:end); %use the other 20% for testing 
%%%%%%%%%%%%%%%%%%% data parsing
...
predictors_train = [resps_train ones(ceil(trainfrac*Ntrials),1)];
...
lambda = 5;
OLE(:,1) = LassoIteratedRidge(predictors_train,stim_train(:,1),lambda);
OLE(:,2) = LassoIteratedRidge(predictors_train,stim_train(:,2),lambda);
%%%%%%%%%%%%%%%%%%%%% train the OLE
disp('OLE trained')
...
predictors_test = [resps_test ones(ntest,1)];
...
lambda = 5;
OQE(:,1) = LassoIteratedRidge(predictors_train,stim_train(:,1),lambda);
OQE(:,2) = LassoIteratedRidge(predictors_train,stim_train(:,2),lambda);
%%%%%%%%%%%%%%%%%%%%% train the OQE
disp('OQE trained')
