...
c_space = .6 ; % distance between stb ceneter nearest neighbors (.3 good default)
c_jitter = .5 ; % jitter off of deterministic grid
c_rand = false ; % make random std centers

% stb dendrite parameters
d_number = 5 ; % number of dendrites (20 good default)
...
d_noise = 25 ; % (degrees) jitter of response (as if d was in different angle than its weighting) 
...
% ds rgc parameters
ds_c_x = 2 ; % center of ds soma
ds_c_y = 2 ; 
ds_nd = 180 ; % (degrees) null direction
...
for d=1:r-1 ; % for each dendrite 
    syn_fact1(d) = 1>sqrt((ds_c_x - d_x(d,end))^2 + (ds_c_y - d_y(d,end))^2) ; % (0,1) end of stb dendrite within range of ds cell?
    
    SomaA(d) = atan2d(ds_c_y - d_y(d,1), ds_c_x - d_x(d,1)) ; % angle of soma (-180:+180)
    SomaDa(d) = abs(atan2d(sind(SomaA(d) - ds_nd), cosd(SomaA(d) - ds_nd))) ; % acute angle between soma and null direction    
    syn_fact2(d) = SomaDa(d)/180 ; % (0-1) gets smaller with smaller acute angle

    dDa(d) = abs(atan2d(sind(d_ang(d) - ds_nd), cosd(d_ang(d) - ds_nd))) ; % acute angle between dendrite and null direction  
    syn_fact3(d) = dDa(d)/180 ; % (0-1) gets smaller with smaller acute angle
    
    d_w(d) = syn_fact1(d) * syn_fact2(d) * syn_fact3(d) ; %(0-1) product of independent weights
end
...
ds_r = (d_r * d_w')/sum(syn_fact1) ; % (0-1) weighted average of dendrites within the rf 
        
%% measuring model output

ds_vector = PolarVectorAddition([bar_angles', ds_r]) ; % vector sum
...
%stb_plot = [1000,2000,3000,4000,5000] ;
stb_plot = [10] ;
