function plot_time_courses_JC(datarun, cell_spec, varargin)
...
% color         b       color of plot
% mean          false   if true than plot the average of all time courses
%
%
% 2009-06  gauthier
...
% 2014-11 JC made changes to add 'color' and 'mean' options
...
p.addParamValue('plot_color','b');
p.addParamValue('mean', true); 
...
% set up plot axes
plot_fig = set_up_fig_or_axes(params.figure);
...
if p.Results.all
    ...
    elseif p.Results.bw
        ...
        if ~isempty(timcs)
                ...
                xlabel('frame number');
                ylabel('contrast');
                set(findobj('Type','line'),'Color',p.Results.plot_color);
        end
    end   
end
