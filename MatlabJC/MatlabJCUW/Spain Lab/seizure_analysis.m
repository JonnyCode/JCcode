function [mean_isi_data,spk_rastor4_zero_data]=seizure_analysis(file,marker);

% this function will filter seizure traces and detect appropriate spikes
% J. Cafaro 6/17/07

% set parameters
%  file = 'i070607cData1' ; % set file to analyze
samplerate = 20000 ;%Hz
...
spk_rastor3(:,1:250) = NaN ; % get rid of stim artifact in sweeps 
spk_rastor4 = reshape(spk_rastor3',1,length(out)); % no stim artifact in one trace

i = find(spk_rastor4==1) ;                      % changes spk_rastor NaNs into zeros
spk_rastor4_zero_data=zeros(1,length(spk_rastor4)) ;
spk_rastor4_zero_data(i)=1 ;
...
for a = 1:number_sweeps       % for each sweep...
    figure(3)
    plot(t(1:sweep_points),spk_rastor2(a,:)*a,marker,'MarkerSize',2)   %plot the spike times
    hold on
end
%________________________________________________________________________
