...
NumTrials = 100 ; % number of repeats 
...
for a = 1:length(NoiseCom_Frac) ; % for each new change in common noise amplitude
    ...
    GexcStd(a,:) = std(Gexc(Trial-NumTrials+1:Trial,:)) ; % std Gexc for this particular set of trials
    GinhStd(a,:) = std(Ginh(Trial-NumTrials+1:Trial,:)) ; % std Ginh for this particular set of trials
    IStd(a,:) = std(I(Trial-NumTrials+1:Trial,:)) ; %
end
...
for a = 1:length(NoiseCom_Frac) ; % for each new change in common noise amplitude
    for b = a*NumTrials-NumTrials+1:a*NumTrials ; % for each trial         
        ...
        ccGRes(b,:) = xcov(GexcRes(b,:),GinhRes(b,:),'coef') ; % cross corr of residuals       
        ccIRes(b,:) = xcov(GexcRes(b,:)*(-45 - params.Eexc),GinhRes(b,:)*(-45 - params.Einh),'coef') ; % cross corr of current residuals
        ...
        InoiseVarPred(b) = mean(var(GexcRes(b,:)*(-45 - params.Eexc),[],2)) + mean(var(GinhRes(b,:)*(-45 - params.Einh),[],2))...
            + ccIRes(b,ccZeroi)*(mean(var(GexcRes(b,:)*(-45 - params.Eexc),[],2)) + mean(var(GinhRes(b,:)*(-45 - params.Einh),[],2))) ; % theoretical prediction based on cross correlation
...
for a=1:length(NoiseCom_Frac) ;
    identifier = ['simulatedGexcMean',num2str(a)] ;
    ForIgor.(identifier) = GexcMean(a,:) ;
    
    identifier = ['simulatedGinhMean',num2str(a)] ;
    ForIgor.(identifier) = GinhMean(a,:) ;

    identifier = ['simulatedIMean',num2str(a)] ;
    ForIgor.(identifier) = IMean(a,:) ;
    
    identifier = ['simulatedGexcStd',num2str(a)] ;
    ForIgor.(identifier) = GexcStd(a,:) ;
    
    identifier = ['simulatedGinhStd',num2str(a)] ;
    ForIgor.(identifier) = GinhStd(a,:) ;
    
    identifier = ['simulatedIStd',num2str(a)] ;
    ForIgor.(identifier) = IStd(a,:) ;   
end

    identifier = ['simulatedccX'] ;
    ForIgor.(identifier) = ccX ;
    
    identifier = ['simulatedcc',num2str(a)] ;
    ForIgor.(identifier) = ccGResMean(a,:) ;
end

for set=1:length(NoiseCom_Frac)  ;
    for a = 1:NumTrials ;
        identifier = ['simulatedSpikeTrain',num2str(a),'set',num2str(set)] ;
        ForIgor.(identifier) = a*stspikes(a+(set-1)*NumTrials,:) ;
    end
end   

identifier = ['simulatedSignal'] ;
ForIgor.(identifier) = signal ;
