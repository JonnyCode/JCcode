...
recFactor_exc = 100 ; % a negative will shift the sine wave into a rectification and positive will bring it out, zero means some of the noise will still be rectified
recFactor_inh = 0 ;
...
% calculate recorded currents and offset them so their mean = zero
I{a}(trial,1:lengthG) = G_exc*(hp-rp_exc) + G_inh*(hp-rp_inh) ;

Ioffset{a}(trial) = mean(I{a}(trial,:),2) ;
I{a}(trial,:) = I{a}(trial,:)-Ioffset{a}(trial) ;
...
% calculate variance of currents
var_Iexc(b) = mean(var(I{1},0,2)) ; 
var_Iint(b) = mean(var(I{2},0,2)) ; 
var_Iinh(b) = mean(var(I{3},0,2)) ; 

% caculate the time dependant variance of the currents and find the means
tdvar_Iexc{b} = var(I{1}) ;
tdvar_Iinh{b} = var(I{3}) ;
tdvar_Iint{b} = var(I{2}) ;

midTdvar_Iexc(b) = max(tdvar_Iexc{b})/8 ;
midTdvar_Iinh(b) = max(tdvar_Iinh{b})/8 ;
midTdvar_Iint(b) = max(tdvar_Iint{b})/8 ;

VarPnts{b} = find(tdvar_Iexc{b}>=midTdvar_Iexc(b) & tdvar_Iinh{b}>=midTdvar_Iinh(b)) ; 
...
var_ResIexc(b) = mean(var(resid_Iexc{b}(:,VarPnts{b}),0,2)) ;
var_ResIint(b) = mean(var(resid_Iint{b}(:,VarPnts{b}),0,2)) ;
var_ResIinh(b) = mean(var(resid_Iinh{b}(:,VarPnts{b}),0,2)) ;
...
var_ResGexc(b) = mean(var(resid_GfromI_exc{b}(:,VarPnts{b}),0,2)) ;
var_ResGinh(b) = mean(var(resid_GfromI_inh{b}(:,VarPnts{b}),0,2)) ;
