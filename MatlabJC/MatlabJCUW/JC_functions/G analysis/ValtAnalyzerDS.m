function ForIgor = ValtAnalyzerDS(Input,Parameters,id,id2,A) ; 
% this function will analyze alternate voltage exp from ds cells presented
% a moving bar

% JC 8/7/09 adapted from earlier version
% editted 12/3/09 rearranged code to filter current and change I to g conversion
% and added a bunch of ForIgors, sampled last pnt instead of 2nd to last,
% editted 2/24/10 added peak of cc
% editted 12/14/10 added meanamp and time of g to calc correlation
% editted 1/24/11 (post paper submition) to get more accurate frame rate 
% editted 6/24/11 testing if noise corr is predictable from exc and inh tuning curves
% editted 7/15/11 to use stim points for length of stim not data length - pre and post pnts as done previously
% editted 7/23/11 changed xcov to xcorr for all resid calcs
...
for a = 1:length(epochs) ;
    ...
    %prePnts(a) = floor((Struct.spatial_prepts-60)/(frameRate*SI(1))) ; % when skips the first trigger
...
if residualOption == 0 ; % added residual option 8/4/10
    ...
    for a= 1:NumBars(1) ;
        ...
        gExc_ON_Res{a} =  gExc_ON{a} - repmat(gExc_ON_Mean{a},size(gExc_ON{a},1),1) ;
        gInh_ON_Res{a} =  gInh_ON{a} - repmat(gInh_ON_Mean{a},size(gInh_ON{a},1),1) ;
        gAlt_Exc_ON_Res{a} = gAlt_Exc_ON{a} - repmat(gAlt_Exc_ON_Mean{a},size(gAlt_Exc_ON{a},1),1) ;
        gAlt_Inh_ON_Res{a} = gAlt_Inh_ON{a} - repmat(gAlt_Inh_ON_Mean{a},size(gAlt_Inh_ON{a},1),1) ;

        gExc_OFF_Res{a} =  gExc_OFF{a} - repmat(gExc_OFF_Mean{a},size(gExc_OFF{a},1),1) ;
        gInh_OFF_Res{a} =  gInh_OFF{a} - repmat(gInh_OFF_Mean{a},size(gInh_OFF{a},1),1) ;
        gAlt_Exc_OFF_Res{a} = gAlt_Exc_OFF{a} - repmat(gAlt_Exc_OFF_Mean{a},size(gAlt_Exc_OFF{a},1),1) ;
        gAlt_Inh_OFF_Res{a} = gAlt_Inh_OFF{a} - repmat(gAlt_Inh_OFF_Mean{a},size(gAlt_Inh_OFF{a},1),1) ;
    end
...
for a= 1:NumBars(1) ;
    ...
    cc_ON(a,:) = xcorr(gExc_ON_Mean{a},gInh_ON_Mean{a},'coef') ;
    ccAlt_ON(a,:) = xcorr(gAlt_Exc_ON_Mean{a},gAlt_Inh_ON_Mean{a},'coef') ; 
    
    cc_OFF(a,:) = xcorr(gExc_OFF_Mean{a},gInh_OFF_Mean{a},'coef') ;
    ccAlt_OFF(a,:) = xcorr(gAlt_Exc_OFF_Mean{a},gAlt_Inh_OFF_Mean{a},'coef') ;
    ...
    for b = 1:size(gExc_Res{a},1) ;
        ...
        ccAltInd_ON{a}(b,:) = xcorr(gAlt_Exc_ON{a}(b,:),gAlt_Inh_ON{a}(b,:),'coef') ;
        ccRes_ON{a}(b,:) = xcorr(gExc_ON_Res{a}(b,:),gInh_ON_Res{a}(b,:),'coef') ;
        ccAltRes_ON{a}(b,:) = xcorr(gAlt_Exc_ON_Res{a}(b,:),gAlt_Inh_ON_Res{a}(b,:),'coef') ;
        ccAltResShuff_ON{a}(b,:) = xcorr(gAlt_Exc_ON_Res{a}(b,:),gAlt_Inh_ON_ResShuff{a}(b,:),'coef') ;

        ccAltInd_OFF{a}(b,:) = xcorr(gAlt_Exc_OFF{a}(b,:),gAlt_Inh_OFF{a}(b,:),'coef') ;
        ccRes_OFF{a}(b,:) = xcorr(gExc_OFF_Res{a}(b,:),gInh_OFF_Res{a}(b,:),'coef') ;
        ccAltRes_OFF{a}(b,:) = xcorr(gAlt_Exc_OFF_Res{a}(b,:),gAlt_Inh_OFF_Res{a}(b,:),'coef') ;
        ccAltResShuff_OFF{a}(b,:) = xcorr(gAlt_Exc_OFF_Res{a}(b,:),gAlt_Inh_OFF_ResShuff{a}(b,:),'coef') ;
    end
    % mean cross corr 
