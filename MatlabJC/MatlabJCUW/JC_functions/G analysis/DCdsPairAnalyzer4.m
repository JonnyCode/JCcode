...
% modified from DCdsPairAnalzer3.m but using entire cross correlation
...
for c = 1:length(cExc_plusConverg_plusPairwise_mean) ; % for every lag
    ...
    for a=1:numBars; % for every bar
        for b=1:numConditions ; % for every set pairings   
            ...
            sn_variance1_predicted(a,b) = Ve1(a,b) + alpha^2*Vi1(a,b) - 2*alpha*Ve1i1(a,b) ;
            sn_variance2_predicted(a,b) = Ve2(a,b) + alpha^2*Vi2(a,b) - 2*alpha*Ve2i2(a,b) ;
            sn_corrcoef_predicted_cc{b}(a,c) = sn_covariance_predicted{b}(a,c)/sqrt(sn_variance1_predicted(a,b)*sn_variance2_predicted(a,b)) ;
        end            
    end
end

for b=1:numConditions ; % for every set pairings
    sn_corrcoef_predicted(:,b) = 2*mean(sn_corrcoef_predicted_cc{b},2) ; % average accross lag 
end
