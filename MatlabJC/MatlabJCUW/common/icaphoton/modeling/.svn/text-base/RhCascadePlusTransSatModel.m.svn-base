...
function ReturnedCondition = RhFeedbackPlusTransSatModel(NumSteps, TransducinSatRate, InitialShutoffRate, Condition, Filter, NumResponses)
...
for resp = 1:NumResponses
	...
	for cnt=1:Condition.EpochPts
		RhTimeCourse(cnt) = CurrentCatalyticActivity * exp(-TransducinSatRate * cnt);
