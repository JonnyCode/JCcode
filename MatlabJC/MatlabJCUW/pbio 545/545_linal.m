...
% ordered collections of points.  The points are ordered so that a
% correspondence can be made between the value and a particular direction
% or axis.  You are all familiar with this from plotting points in
% cartesian (x-y-z) coordinates, where you can represent a point as
% (x,y,z).  This is a general concept.  For example, Starbucks could choose 
% to represent different types of espresso drinks as collections of two
% numbers - one for the amount of milk and the other for the amount of
% espresso. (This is at least how it should be - i.e. before you could
% choose different types of milk, different flavors, ... .  Those would add
% more axes):
...
% [0.5 0.5].  We will mostly use column vectors.  The difference between
% row and column vectors is important mainly when we start thinking about
% matrices (below).

% The advantage of using vectors to represent data is that it allows us to
% visualize the data (provided it has low enough dimension).  Also, as we
% will see below, we can perform mathematical operations on vectors using
% matrices - in many instances this very efficient and even intuitive.
...
% (If Matlab won't run the above lines make sure PlotVector is on your
% Matlab path - try typing 'which PlotVector' in the command window if you
% are not sure).  
...
% algebra course, is to solve systems of linear equations.  For example,
% what combination (x,y,z) satisfy
%	2x + 3y - 3z = 1 
%   -x + y = 2 
%   3x - y + z = -1
% These are linear because x, y, and z occur to power one (i.e. there are
% no x^2 or x^0.5, etc, terms) and are not multiplied by each other.
...
% fails there is either no exact solution to the set of equations, or their
% are multiple solutions.  All is not lost necessarily though, as we often
% can find an approximate solution (using e.g. singular value
% decomposition, see below).
% 	One approach to solving this set of equations for x, y and z is to
% solve one equation for one of the variables and substitute into the other
% two equations.  If we do this twice we eliminate two of the variables and
% get a simple algebraic equation for the last.  Then we can work backwards
...
% other), but you can see that the two vectors lie in a plane.  The section
% below formalizes these ideas a bit
...
% x+y and x-y.  The column and null space of a matrix determine the set of
% directions or region of space spanned by the columns in the matrix (the
...
% eigensystem of a matrix.  That is the topic of this and the next section.
...
% other choices of coordinates.  Below we'll consider how to find the 
% eigenvalues and eigenvectors and illustrate their use with a simple
...
% You can find eigenvalues analytically from the matrix determinant.  For more
% about this see a linear algebra book (e.g. Strang, Linear Algebra and its
% Applications).  Matlab will also numerically determine the eigenvalues and 
% eigenvectors for you.  Start with a matrix:
...
% which is indeed v times the first eigenvalue (which is 1).  Let's try another
